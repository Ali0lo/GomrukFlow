    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <div className="p-5 border-b border-slate-200 bg-slate-900 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center space-x-2 no-print">
          ...
          >
            {copied ? (
              ...
              <>
                ...
                <span>{lang === "az" ? "JSON Kopyala" : "Copy JSON"}</span>
              </>
            )}
          </button>
          ...
          <button
            onClick={handlePrint}
            className="flex items-center space-x-1.5 px-3 py-1.5 text-xs font-semibold bg-brand-600 hover:bg-brand-500 text-white rounded-lg shadow-sm transition-colors"
          >
            <Printer className="w-3.5 h-3.5" />
            <span>{lang === "az" ? "Çap et / PDF Saxla" : "Print / PDF"}</span>
          </button>
        </div>
      </div>

      {/* Official-style Disclaimer Banner */}
      <div className="p-4 bg-amber-50 border-b border-amber-200 text-amber-900 text-xs flex items-start space-x-2.5">
      ...
      {/* Grid of Declaration Boxes (laid out like modern digital customs filing summary) */}
      <div className="p-5 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 rounded-xl border border-slate-200 bg-slate-50/50">
            ...
            <div className="font-bold text-slate-900 text-sm">{boxes.box_2_exporter.name}</div>
            <div className="text-xs text-slate-600 mt-1">
              VÖEN / TIN: <span className="font-mono font-bold text-slate-800">{boxes.box_2_exporter.voen}</span>
            </div>
            <div className="text-xs text-slate-500 mt-0.5">{boxes.box_2_exporter.address}</div>
            <div className="text-xs font-semibold text-slate-700 mt-1">{boxes.box_2_exporter.country}</div>
          </div>
          ...
          <div className="p-4 rounded-xl border border-slate-200 bg-slate-50/50">
            ...
            <div className="font-bold text-slate-900 text-sm">{boxes.box_8_consignee.name}</div>
            <div className="text-xs text-slate-500 mt-1">{boxes.box_8_consignee.address}</div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
          <div className="p-3 rounded-lg border border-slate-200 bg-white">
            ...
            <span className="font-bold text-slate-900 mt-0.5 block">{boxes.box_1_declaration_type.value}</span>
          </div>
          ...
          <div className="p-3 rounded-lg border border-slate-200 bg-white">
            ...
            <span className="font-bold text-slate-900 mt-0.5 block">{boxes.box_20_delivery_terms.value}</span>
          </div>
          ...
          <div className="p-3 rounded-lg border border-slate-200 bg-white">
            ...
            <span className="font-mono font-bold text-brand-700 mt-0.5 block text-sm">
          ...
          <div className="p-3 rounded-lg border border-slate-200 bg-white">
            ...
            <span className="font-bold text-slate-900 mt-0.5 block">{boxes.box_25_transport.value}</span>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-slate-500 font-medium">
              Cəmi Netto: <b className="text-slate-800">{summary_totals.total_net_weight_kg.toLocaleString()} kg</b> | Brutto: <b className="text-slate-800">{summary_totals.total_gross_weight_kg.toLocaleString()} kg</b>
            </span>
          </div>
              ...
              <tbody className="divide-y divide-slate-200 font-medium text-slate-800 bg-white">
                {declaration_items.map((item, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/70">
                    ...
                    <td className="p-3">
                      <div className="font-bold text-slate-900">{item.box_31_description}</div>
                      <div className="text-[11px] text-slate-500">{item.box_31_packages}</div>
                    </td>
        ...
        <div className="p-4 rounded-xl border border-slate-200 bg-slate-50/50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 text-xs">
            {boxes.box_44_documents.documents.map((doc: any, idx: number) => (
              ...
              >
                <div>
                  <span className="font-mono font-bold text-slate-600 text-[11px] mr-1.5">
                    [{doc.customs_code}]
                  </span>
        ...
        <div className="p-4 rounded-xl border border-slate-200 bg-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
          <div>
            ...
            <p className="text-slate-500 text-[11px]">{boxes.box_14_declarant.note}</p>
          </div>

          <div className="px-4 py-2 bg-slate-100 rounded-lg text-slate-600 text-center font-mono text-[11px] border border-slate-200 shrink-0">
